...
options and then deliver the values to the programmer at the end of
...
In the ~examples~ directory, after building the examples with either
~alr build~ or ~gprbuild -P examples.gpr~, execute
~scripts/check-args-simple~ and ~scripts/check-args-simple2~ to see
those two programs executed with most of their options, and see the
programs' outputs with those options.
- [[file:examples/src/simple.adb][simple.adb]] excercises all the option types, with all
  ...
  #+END_SRC
- [[file:examples/src/simple2.adb][simple2.adb]] (and the associated [[file:examples/src/simple2_args.ads][simple2_args.ads]] and
  [[file:examples/src/simple2_args.adb][simple2_args.adb]] package) shows how to use this if you don't want to
  use GNAT's ~'Unrestricted_Access~.  It has exactly the same argument
  parsing logic as simple.adb, just separated out into a package so it
  can use ~'Access~.
